...
- `BREVO_MAX_MESSAGE_VERSIONS` (optionnel, défaut 100) : taille de chunk pour l’envoi (1 destinataire par messageVersion)
...
- **Notifications et campagnes** : l’intégration Brevo est **safe-by-default** :
  ...
  - Workflow recommandé : `sync_contacts` (prépare les contacts) → `/campaigns/preview` (prévisualise, jamais de réseau) → `/campaigns/send` (envoie le lot 200–300 contacts, dry-run si pas de clé). Les tests/CI restent offline (aucun trafic sortant).
  - Confidentialité : 1 destinataire par `messageVersion` (pas de TO multiples), chunking par `BREVO_MAX_MESSAGE_VERSIONS` (par défaut 100) avec retry/backoff 429.
  - En cas d’erreur HTTP (ex. 429), un backoff/retry minimal est appliqué avant d’échouer proprement.

### Campagnes: endpoints clés

- `POST /campaigns/preview` : sélectionne les contacts éligibles du dernier run (ou `run_id` explicite), applique filtres optionnels (`segment` RFM, `cluster`), limite par `batch_size` (200–300), renvoie `preview` (5 contacts), `n_selected`, `n_in_batch`, et reste toujours hors-ligne.
- `POST /campaigns/send` : même sélection, déclenche `send_batch` (dry-run si `BREVO_DRY_RUN=1` ou clé absente), retourne `count`, `api_calls`, `dry_run`, `preview`.

Flux UI : lancer un run reco → page *Campagnes* → saisir `template_id`, éventuellement filtre segment/cluster → **Prévisualiser** (jamais de réseau) → **Envoyer** (safe-by-default).
