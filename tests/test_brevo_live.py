...
def test_real_client_retries_on_429(monkeypatch):
    ...
    result = client.send_batch(
        {"template_id": "tpl", "contacts": [{"email": "a@test.com", "customer_code": "C1"}], "batch_id": "b1", "run_id": "r1"}
    )
    assert calls["count"] == 2
    assert result["status"] == "sent"


def test_send_batch_uses_full_contacts_and_preview(monkeypatch, tmp_path):
    monkeypatch.setenv("BREVO_DRY_RUN", "0")
    db_url = f"sqlite:///{tmp_path/'brevo_full_contacts.db'}"
    db_module, models, _ = _setup(db_url)
    SessionLocal = sessionmaker(bind=db_module.engine)
    db = SessionLocal()
    tenant = models.Tenant(name="t1", domain=None)
    ...
    for idx in range(1, 8):
        clients.append(
            models.Client(client_code=f"C{idx}", name=f"User{idx}", email=f"user{idx}@test.com", tenant_id=tenant.id)
        )
    run, summary, eligible = _minimal_run(db_module, models, tenant.id)
    # Make all clients eligible
    eligibles = [models.NextActionOutput(run_id="run-live", customer_code=c.client_code, eligible=True, tenant_id=tenant.id) for c in clients]
    db.add_all([run, summary] + eligibles + clients)
    db.commit()
    ...
    assert resp["count"] == len(clients)
    assert len(resp["preview"]) == 5
    assert len(captured["contacts"]) == len(clients)
    db.close()


def test_real_client_chunks_per_max_versions(monkeypatch):
    calls = {"count": 0}
    ...
    monkeypatch.setenv("BREVO_MAX_MESSAGE_VERSIONS", "2")
    client = brevo_service.RealBrevoClient("key", http_post=fake_post, base_url="http://example.test")
    contacts = [{"email": f"c{i}@test.com", "customer_code": f"C{i}"} for i in range(5)]
    result = client.send_batch({"template_id": "tpl", "contacts": contacts, "batch_id": "b1", "run_id": "r1"})
    assert calls["count"] == 3  # ceil(5/2)
    assert result["api_calls"] == 3
    for body in result["responses"]:
        assert body.get("ok") is True


def test_real_client_message_versions_one_recipient_each(monkeypatch):
    calls = {"payloads": []}
    ...
    client = brevo_service.RealBrevoClient("key", http_post=fake_post, base_url="http://example.test")
    contacts = [
        {"email": "a@test.com", "customer_code": "C1"},
        {"email": "b@test.com", "customer_code": "C2"},
    ]
    client.send_batch({"template_id": "tpl", "contacts": contacts, "batch_id": "b1", "run_id": "r1"})
    assert len(calls["payloads"]) == 1
    payload = calls["payloads"][0]
    versions = payload["messageVersions"]
    assert len(versions) == 2
    assert all(len(v["to"]) == 1 for v in versions)
